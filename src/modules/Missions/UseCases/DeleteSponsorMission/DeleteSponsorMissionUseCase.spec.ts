...
describe("Delete sponsor mission",()=>{
  ...
  it("Shouldn't be able delete a sponso with invalid values",async()=>{
    expect(async()=>{
      await deletedSponsorMissionUseCase.execute("","")
  ...
  it("Should be able delete a sponsor mission",async()=>{
    ...
    const deleteSponsorMission = await deletedSponsorMissionUseCase.execute(createSponsorMission.id_sponsor,createSponsorMission.id_mission)
