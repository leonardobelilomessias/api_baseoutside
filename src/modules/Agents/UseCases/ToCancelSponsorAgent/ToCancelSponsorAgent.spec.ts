...
describe("To Cancel SponsorAgent", () => {
  ...
  it("Should be able to cancel a sponsor agent",async  () => {
   const agent1 = await agentInMemoryRepository.create({name:"agent1",email:"agent1@email",password:"xxx"})
    const agent2 = await agentInMemoryRepository.create({ name: "agent2", email: "agent2@email", password: "xxx" })
