...
describe("List Colabs Agent", () => {
  ...
  it("Should be able list all colabs agents", async () => {
    const agent1 = await agentRepositoryInMemory.create({ name: "agent1", email: "agent1@email", password: 'xxx' })
    const agent2 = await agentRepositoryInMemory.create({ name: "agent2", email: "agent2@email", password: 'xxx' })
    const agent3 = await agentRepositoryInMemory.create({ name: "agent3", email: "agent3@email", password: 'xxx' })
