...
import { IEditAgentDTO, IEditAgentInputDTO, IResponseAgentDTO } from "../../DTOS/IAgentDTOS"
...
class UpdateAgentUseCase{
  ...
  async execute({id_agent_token, id, name, description, email, interests, skills, vocation }:IEditAgentInputDTO):Promise<IResponseAgentDTO> {
    ...
    const agentToEdit = await this.agentRepository.edit(agentExist as IEditAgentDTO)
    return agentToEdit
  }
